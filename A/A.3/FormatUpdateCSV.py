...
def score_row(row: Dict[str, str], kind: str, stats: Dict[str, Any]) -> float:
    ...
    repeated_authors = sum(1 for author_name in authors if author_freq[author_name] >= 2)
    very_repeated_authors = sum(1 for author_name in authors if author_freq[author_name] >= 3)
...
def select_curated_rows(
    ...
    candidates = [row for row in rows if is_valid_row(row, kind)]
...
class GraphBuilder:
        ...
        if venue_id is None:
            ...
            self.conference_workshops.setdefault(
                venue_id,
                {
                    "venueId": venue_id,
                    "name": venue_name,
                    "reviewerPolicyNumber": self.reviewers_per_paper,
                },
            )
    ...
    def write_csv(self, out_dir: str) -> None:
        ...
        log_step("Writing organizations.csv")
        ...
        log_step("Writing conference_workshops.csv")
        self._write(
            out_dir,
        ...
        log_step("Writing affiliated_with.csv")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate a curated graph CSV dataset for A.3 while preserving the extended schema."
    )
    ...
    output_dir = project_root / "data" / "csv_graphmodel_A3_data"
    ...
    for index, row in enumerate(selected_articles, start=1):
        builder.process_article_row(row)
        if index % 10 == 0 or index == total_articles:
            log_step(f"Processed articles: {index}/{total_articles}")

    log_step("Building graph from selected inproceedings...")
    total_inproceedings = len(selected_inproceedings)
    for index, row in enumerate(selected_inproceedings, start=1):
        builder.process_inproceedings_row(row)
        if index % 10 == 0 or index == total_inproceedings:
            log_step(f"Processed inproceedings: {index}/{total_inproceedings}")
    ...
    if proceedings_path and proceedings_header_path:
        ...
        selected_venues = {row_container(row, "inproceedings") for row in selected_inproceedings}
        selected_years_by_venue = defaultdict(set)
        for row in selected_inproceedings:
            selected_years_by_venue[row_container(row, "inproceedings")].add(row_year(row))

        matched = 0
        total_proceedings = len(proceedings_rows)
        for index, row in enumerate(proceedings_rows, start=1):
            ...
            if index % 200 == 0 or index == total_proceedings:
                log_step(f"Proceedings scan progress: {index}/{total_proceedings}")
